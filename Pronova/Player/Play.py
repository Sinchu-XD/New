...
class Player:
    ...
    async def play(self, chat_id: int, song, video: bool = False) -> int:
        LOGGER.info(f"[PLAY REQUEST] {song.title!r} in {chat_id}")

        try:
            q = self._queue(chat_id)
            pos = q.add(song)

            LOGGER.info(f"[QUEUE ADD] {song.title!r} at position {pos}")
            ...
            if pos == 1:
                try:
                    ...
                    song.position = 0
                    LOGGER.info(f"[STREAM STARTED] {song.title!r}")

                except Exception:
                    LOGGER.error(f"[PLAY ENGINE ERROR] {song.title!r}\n{format_exc()}")
                    q.pop_last()
    ...
    async def skip(self, chat_id: int):
        ...
        try:
            ...
            nxt = q.next()
            LOGGER.info(f"[NEXT] {nxt.title!r if nxt else 'queue empty'}")
            ...
            if not stream or not stream.startswith("http"):
                LOGGER.warning(f"[SKIP] Stream missing for {nxt.title!r}, using prefetch cache")
            ...
            LOGGER.info(f"[PLAYING NEXT] {nxt.title!r}")
            return nxt
    ...
    async def previous(self, chat_id: int):
        ...
        try:
            ...
            LOGGER.info(f"[PLAYING PREVIOUS] {prev.title!r}")
            return prev
    ...
    async def volume(self, chat_id: int, volume: int):
        await self.engine.change_volume(chat_id, max(0, min(volume, 200)))
